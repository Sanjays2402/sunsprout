...
**STATUS:** Tick #12 complete. 60 autoship features now on `main`. This batch shipped storm shelter pairing (two shelters within Chebyshev 2 widen each one's coverage from 3x3 to 5x5; both still consumed; effectiveRadius snapshots before mutating the array; sprite renders a paired sigil), rare fertilizer (per-season rare day from a 32-bit hash of season+1 lands batches into a new RARE_FERTILIZER_INVENTORY_KEY; rare bags grant +4 streak vs regular +2; applyFertilizer prefers rare bags first), heritage chickens (HERITAGE_HATCH_RATE=18% per hatched chick at deterministic rollHeritage(tx,ty,day); coop.heritage[] per-slot flag; coopTick adds HERITAGE_FANCY_BONUS=0.15 to per-chicken fancy rate; pendingHeritage flows through claimPendingChicken), pond species rotation (new reseedPond() + 'restocked' PondOutcome variant; interactPond auto-tries a swap when the pond is stocked-but-empty AND the player carries a different fish; status line hints the swap path), and bath house spa pass (700g punch card from Pip's cart carries SPA_PASS_PUNCHES=4 free soaks; takeBath auto-redeems then prefers punches over gold; bathFlavorLine names "spa pass, N punches left"; persists through SaveSnapshot.player.spaPass). 1242/1242 tests green (+53 this batch); build 235.40 kB / 71.32 kB gz.
...
- [ ] **Heritage breeding** — two heritage chickens in the same coop occasionally pass an egg with a heritage roll-through so the line propagates without buying more passes.
- [ ] **Rare-fertilizer HUD glance** — show today's rare-day countdown in the bin status line so the player can plan deposits across the season.
- [ ] **Pond decor upgrades** — buy a stone-rim pond kit from Pip that raises POND_MAX_PENDING from 6 to 10.
- [ ] **Spa pass loyalty card** — bath house tracks total soaks; gift a free perfumed soap cosmetic at every 10 soaks.
...
(none. Five recurring observations:
 1. KEY BINDINGS STAYED PUT THIS TICK. No new keybinds — every feature extended an existing flow (`>` for the pond, `7` for compost/fertilizer apply, E for the bath house, dawn ticks for storm + hatchery + compost). Digits 9 and 0 still free; letter keys still exhausted.
 2. BATH OUTCOME SCHEMA. takeBath's 'soaked' outcome now ALSO carries `paidWithPass` + `passesLeft` on top of last tick's `pricePaid` + `discounted`. Both bath-house.test.ts AND bath-winter-discount.test.ts inline fixtures had to be updated this tick — any future caller that builds a 'soaked' outcome inline needs all four fields.
 3. SHELTER COVERAGE API. isUnderShelter() and consumeShelteringShelters() now route through effectiveRadius() which depends on the live shelters[] array. Any future caller that fakes shelters by mutating world.shelters mid-loop needs to call effectiveRadius() AFTER staging the final array. The consume path already snapshots radii before iterating to dodge this.
 4. COMPOST TICK SIGNATURE. compostTick(world, player, today) gained an optional `season` arg (default 0). Old callers still compile but lose the rare-bag distinction — game.ts now passes time.season. Tests that depend on REGULAR bag yields should pass a non-rare season (spring=3, summer=6, fall=5, winter=2) or use a finishOnDay that doesn't match the rare day for season 0.
 5. COOP HERITAGE FLAG. PlacedCoop gained `heritage?: boolean[]` (per-slot). addChicken(coop, heritage?) now writes coop.heritage[coop.chickens] before incrementing. Older saves restore with the field undefined and behave exactly like before — heritageCount() returns 0 and isHeritageChicken() returns false.)

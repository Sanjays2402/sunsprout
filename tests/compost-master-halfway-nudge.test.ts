...
describe('compostLedgerLine — pulper halfway nudge', () => {
  it('fires AFTER compost-master is earned, at the bag-count floor', () => {
    const p = {} as object;
    // Earn compost-master quickly via rare bags (3g each), then top
    // up regular bags to reach the pulper nudge floor.
    for (let i = 0; i < 34; i++) recordApplied(p, COMPOST_RECYCLE_RARE);
    // 34 rare = 102g recycled, 34 bags. Compost-master earned.
  ...
  it('names the bag-count runway in the tail', () => {
    const p = {} as object;
    // Earn compost-master quickly via rare bags.
    for (let i = 0; i < 34; i++) recordApplied(p, COMPOST_RECYCLE_RARE);
    // Then exactly 200 regular bags more → 234 lifetimeBagsApplied.
    for (let i = 0; i < 200; i++) recordApplied(p, COMPOST_RECYCLE_REGULAR);
    const line = compostLedgerLine(p);
  ...
  it('handles singular phrasing when exactly 1 bag remains', () => {
    const p = {} as object;
    // Earn compost-master via rare bags first.
    for (let i = 0; i < 34; i++) recordApplied(p, COMPOST_RECYCLE_RARE);
...
describe('compostLedgerLine — nudge mutual exclusion', () => {
  it('does NOT fire BOTH nudges in the same line', () => {
    // Pile compost-master gold via rare bags first → compost-master nudge OFF (earned), pulper nudge eligible by bag count.
    const p = {} as object;
    for (let i = 0; i < 34; i++) recordApplied(p, COMPOST_RECYCLE_RARE);
    ...
    const line = compostLedgerLine(p);
    // Only the pulper tail should appear — the compost-master tail
    // requires recycled gold in [50, 100) which is already past.
    expect(line).toContain('to the pulper badge');
